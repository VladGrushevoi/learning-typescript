...
const clockDiv = document.getElementsByClassName("container-clock")[0];
const hexNumbers = ["A", "B", "C", "D", "E", "F"];
setInterval(() => {
    const dayName = new Date().toLocaleString("en-US", { weekday: "long" });
    const hexLettersFromDay = dayName.split("").map(char => {
    ...
    let colorString = "#" + timeNow.toString().replace(":", "").replace(":", "");
